...
param([string]$mUCPath,[string]$summaryFilesPath, [double] $ServiceMultiplier = 1/30)
##### CLASSES
...
Function Get-KPIs{
 ...
 if (Test-Path -Path $serviceLPFile ){
    # OK, there is file
        ...
        Write-Error "Could not find $serviceLPFile"
        Exit
 }
 ...
 if (Test-Path -Path $LP_KPItimeStampFile ){
  ...
  foreach($kEy in $timeStampsHashLP.Keys){
   ...
   # init push/stream data set with 5 days data or other value
  [hashtable]$timeStampsHashLP = @{}
  $minDate = (Get-Date).AddHours(-9)

  # ****** update data below with real user id and password, as well, as with real workspace, dataset Id and table data
  
  
  Write-Host("start  new stream file from $minDate")
    # reset dataset - log in
    $password = "LukaNino5559*pe" | ConvertTo-SecureString -asPlainText -Force
    $username = "mirko.jelcic@matrix99.net"
    $credential = New-Object System.Management.Automation.PSCredential($username, $password)
    Connect-PowerBIServiceAccount -Credential $credential
    # data set created by powerbi service
    $dataSetID = "232d1c6f-9cae-4da9-a748-a879d006b935"
    $tableName = "PushLachNer"
    #The below URL should be used for My Workspace datasets
    $myWorkSpaceUrl = 'https://api.powerbi.com/v1.0/myorg/groups/3e8b7b44-0201-4a4a-af8d-e8a0313af7ed/datasets/' + $datasetId + '/tables/' + $tableName + '/rows'
...
try{
  ...
  $endpoint = "https://api.powerbi.com/beta/fbd26fd4-e0f5-4bf0-b237-9be32fc209bc/datasets/232d1c6f-9cae-4da9-a748-a879d006b935/rows?noSignUpCheck=1&key=emOjOi3Zok3PaAEp8IoRT67eJozqyfrKuvZ9nfJuAtK%2FaQkHT0Py3ldtj0fDcRqp3I8iH469%2BVgGay3WIWPskA%3D%3D"
